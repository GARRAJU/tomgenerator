...
def is_junk_table(name: str) -> bool:
    ...
    if name in ["clipboard", "csv"]:
        return True

    return False


def clean_visual_column_name(name: str):
        ...
        with Connection(endpoint=hyper.endpoint, database=hyper_path) as conn:
            ...
            for schema in schemas:
                ...
                for table in table_list:
                    ...
                    columns = [clean(str(c.name)) for c in table_def.columns]
...
"""
    ...
    def execute(self, folder_name):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            xml_tables, local_name_map = self.extract_xml_metadata(root)

            try:
                hyper_tables = read_hyper_tables(twbx_path)
                tables = hyper_tables if hyper_tables else xml_tables
            except:
                tables = xml_tables
